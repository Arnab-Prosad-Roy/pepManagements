        ...
        <div style="padding-left: 70px;padding-top: 20px;padding-right: 80px;">
          <div class="row">
            <div style="width: 550px;float: left;">
             ...
             <h3 style="margin-top:-18px;font-size: 26px;">Letter of Introduction</h3><br>
              <h3 style="margin-top:-18px;font-size: 18px;">INCUMBENT CERTIFICATE</h3><br>
...
?>
    ...
    if ($getPadd) {
     ...
     $level = $rw_per['level'];
     $floor = $rw_per['floor'];
     $holding = $rw_per['holding'];
     $building = $rw_per['building'];
     $roadName = $rw_per['roadName'];
     $road = $rw_per['road'];
     $block = $rw_per['block'];
     $sector = $rw_per['sector'];
     $zone = $rw_per['zone'];
     $section = $rw_per['section'];
     $area = $rw_per['area'];
     $village = $rw_per['village'];
     $divId = $rw_per['division'];
     $distId = $rw_per['district'];
     $thId = $rw_per['thana'];
     $postId = $rw_per['postoffice'];
     $divName = $rw_per['divName'];
     $distName = $rw_per['distName'];
     $thName = $rw_per['thName'];
     $postName = $rw_per['postName'];
     $postCode = $rw_per['postCode'];
...
?>
              ...
              <?php if($building){echo $building.' ';}?><?php if($holding){echo 'Holding# '.$holding.' ';}?><?php if($flat){echo 'Flat-'.$flat.' ';}?><?php if($suit){echo $suit.' ';}?>
               <?php if($level){echo 'Level-'.$level.' ';}?><?php if($floor){echo $floor.' Fl';}?>
               
               <?php if($roadName){echo $roadName.' ';} ?><?php if($road){echo ' Road# '.$road.' ';}?><?php if($block){echo 'Block-'.$block.' ';}?><?php if($sector){echo 'Sector#'.$sector.' ';}?><?php if($zone){echo 'Zone#'.$zone.' ';}?><?php if($section){echo 'Section#'.$section;}?><?php if($area){echo $area.' ';}?>
               <?php if($thName){echo 'P/S- '.$thName.' ';}?><?php if($postId){echo 'P/O- '.$postName.' ';}?>
               <?php if($distName){echo $distName.'-';}?><?php if($postCode){echo $postCode;}?>, Bangladesh
...
<?php if($years <= $ye AND $mo <=$mn) {?>
  ...
  <td>0</td>
    ...
    <a href="create_letter_of_intro.php">
